---
identifier: "PAP-489"
title: "Wire tables behind the module registry with an adapter and feature flag"
project: "tables"
...
Make `tables` a swappable module in practice: its implementation binds into the `@paperos/kernel` registry as the default adapter for `@paperos/contract-tables`, every consumer resolves the ports from the kernel instead of importing `packages/views`, and a `module.tables.impl` variant flag can select a second implementation per tenant with shadow-run and five-second rollback (`docs/module-system.md` sections 4 and 6). After this issue, rewriting tables touches no other module.

**Scope**

In:

* `packages/<impl>/src/adapter.ts` (or one file per port): classes or factories implementing each port from `@paperos/contract-tables`, registered by `module.ts` `bind(kernel)`; request-scoped ports receive the tenant and actor from the kernel's request scope.
* Consumer migration: every import of `tables` internals from another module (found by `pnpm gen:dep-map --undeclared`) replaced with `kernel.resolve(port)` or a hook from the contract; lint R8 turned from warning to error for this module.
* Flag `module.tables.impl` declared in `flags.yaml` (PAP-366) with variants `default` and `next`; a `next` implementation stub that re-exports the default (so the mechanism is exercised before a real rewrite exists).
...
Provides: `tables` bound in the kernel as `@paperos/contract-tables` provider `default`, flag `module.tables.impl`, manifest `slots.fills` and `events.subscribes` complete, `X-PaperOS-Impl` on this module's routes. Consumes: `@paperos/kernel` registry and DI (`module-system/registry-di`), flag swap mechanism (`module-system/flag-swap`), `@paperos/contract-tables@0.1` and its conformance suite, the implementation from PAP-337, PAP-340, PAP-343, PAP-366 flags, PAP-267 middleware for the header. Consumed by: every consumer of the contract (PAP-102 (PM boards), PAP-183 (finance reports as views), PAP-189 (CRM views), PAP-333 (record features), PAP-347 import mapping, PAP-207 template packs, PAP-113 org chart), the swap CLI, PAP-266's removal matrix.

**Test plan**

* Unit: `bind` registers every port; unbound port fails boot with its name; request scope carries tenant and actor.
* Integration (compose stack): conformance suite passes against the kernel-resolved provider (not a direct import); flipping `module.tables.impl` to `next` for one tenant changes `X-PaperOS-Impl` for that tenant only within 5 s; kill switch returns `default`.
* Static: `pnpm gen:dep-map --undeclared` reports zero undeclared edges into `tables`; lint R8 error level; `knip` finds no unused exports left behind by the migration.
...
* Kernel binding merged; zero undeclared imports into `tables` across the monorepo; conformance green through the kernel; flag flip recording attached; shadow diff report empty.
...
Built by Nova (Table & Views Engine owner). Reviewed by Forge and Forge (kernel).
...
Reviewer flips `module.tables.impl` to `compiler-v2` (a copy) for the demo tenant, opens the CRM pipeline and the PM board, and both render identically with `X-PaperOS-Impl: compiler-v2` in the response headers; flips back. Ninety seconds.

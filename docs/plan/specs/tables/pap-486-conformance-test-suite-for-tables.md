---
identifier: "PAP-486"
title: "Conformance test suite for tables contract"
project: "tables"
...
Give `@paperos/contract-tables` an executable meaning: a Vitest suite that any implementation of the tables ports must pass, plus the golden fixtures both old and new implementations are diffed against during a swap (`docs/module-system.md` section 5). Without this, "plug and play" is a promise; with it, `paperos module conformance tables --impl a,b` is a fact the gates can check.

**Scope**

In:

* `packages/contracts/tables/conformance/`: `defineConformanceSuite('tables', factory)` covering every port exported at v0.1, one `describe` per port, one `it` per fixture; `unsupported` capability handling so partial adapters report honestly.
* `fixtures/`: ten golden `ViewSpec`s (one per kind) over two datasets with expected rows, cursors and aggregates; twelve field type samples; six formulas with expected types and SQL; two automation triggers; every fixture validated against the contract schemas in CI.
...
* Every case id is stable (`tables.<port>.<n>`) so shadow-run diffs and Linear comments can cite it.

**Interface contract**

Provides: `@paperos/contract-tables/conformance` (`defineConformanceSuite`, `memory` doubles, `fixtures` index, `capabilities` list), `conformance.json` artefact for this module. Consumes: `@paperos/contract-tables@0.1` (`module/tables/contract`), the runner and artefact schema (`module-system/conformance-runner`, PAP-239), the PAP-42 compose stack for the real-adapter run, `callAs` from PAP-268 where routes are exercised. Consumed by: `module/tables/wire`, the swap CLI, Gate 1, and every future implementation of the contract.

**Test plan**

* The suite passes against the memory doubles in CI on every PR that touches the contract or the module.
* The suite passes against the real implementation (PAP-337, PAP-340, PAP-343) on the compose stack nightly; failures open a Linear comment on the implementation issue with the case ids.
...
* Registered with the runner; Gate 1 step `conformance:tables` green; `conformance.json` attached to the PR.
* `docs/platform/contracts/tables.md` gains a "Conformance" section listing case ids; Linear comment with the report.
...
Blocked by `module/tables/contract` and `module-system/conformance-runner`. Blocks `module/tables/wire`. Soft: PAP-337, PAP-340, PAP-343 for the real-adapter run (the memory run is enough to merge).
...
Reviewer runs the suite against the Postgres compiler on the compose stack and against the in-memory query engine in `conformance/`; both return identical rows, cursors and aggregates for all ten golden views. Two minutes.

---
identifier: "PAP-457"
title: "Conformance test suite for identity contract"
project: "identity"
...
parent: null
children: []
blockedBy: ["PAP-441", "PAP-456", "PAP-541"]
blocks: ["PAP-458"]
key: "module/identity/conformance"
url: "https://linear.app/paperos/issue/PAP-457/conformance-test-suite-for-identity-contract"
source: "Linear snapshot 2026-09-18T14:58Z (plan/linear-snapshot-live.json)"
updatedAt: "2026-09-18T14:21:06.249Z"
model: "claude-opus-5"
effort: "high"
estimate: 3
dueDate: "2026-09-25"
...
# PAP-457: Conformance test suite for identity contract

**Model / Effort:** Opus 5 / high

**Goal**

Give `@paperos/contract-identity` an executable meaning: a Vitest suite that any implementation of the identity ports must pass, plus the golden fixtures both old and new implementations are diffed against during a swap (`docs/module-system.md` section 5). Without this, "plug and play" is a promise; with it, `paperos module conformance identity --impl a,b` is a fact the gates can check.

**Scope**

In:

* `packages/contracts/identity/conformance/`: `defineConformanceSuite('identity', factory)` covering every port exported at v0.1, one `describe` per port, one `it` per fixture; `unsupported` capability handling so partial adapters report honestly.
* `fixtures/`: the five base roles times four audiences as a permission matrix (80 cases), six policy explain traces, three tenant switch sequences, two agent key scopes, one impersonation with audit expectation; every fixture validated against the contract schemas in CI.
...
* Every case id is stable (`identity.<port>.<n>`) so shadow-run diffs and Linear comments can cite it.

**Interface contract**

Provides: `@paperos/contract-identity/conformance` (`defineConformanceSuite`, `memory` doubles, `fixtures` index, `capabilities` list), `conformance.json` artefact for this module. Consumes: `@paperos/contract-identity@0.1` (`PAP-456`), the runner and artefact schema (`PAP-441`, PAP-239), the PAP-42 compose stack for the real-adapter run, `callAs` from PAP-268 where routes are exercised. Consumed by: `PAP-458`, the swap CLI, Gate 1, and every future implementation of the contract.

**Test plan**

* The suite passes against the memory doubles in CI on every PR that touches the contract or the module.
* The suite passes against the real implementation (PAP-223, PAP-227, PAP-229, PAP-58) on the compose stack nightly; failures open a Linear comment on the implementation issue with the case ids.
...
* Registered with the runner; Gate 1 step `conformance:identity` green; `conformance.json` attached to the PR.
* `docs/platform/contracts/identity.md` gains a "Conformance" section listing case ids; Linear comment with the report.
...
Blocked by `PAP-456` and `PAP-441`. Blocks `PAP-458`. Soft: PAP-223, PAP-227, PAP-229, PAP-58 for the real-adapter run (the memory run is enough to merge).
...
Reviewer runs the suite against Better Auth plus the SQL predicate compiler on the compose stack and against the memory doubles; both pass; the reviewer flips one row in the matrix fixture and both fail on the same case id. Two minutes.

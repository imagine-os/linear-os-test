---
identifier: "PAP-494"
title: "Conformance test suite for migration contract"
project: "migration"
...
Give `@paperos/contract-migration` an executable meaning: a Vitest suite that any implementation of the migration ports must pass, plus the golden fixtures both old and new implementations are diffed against during a swap (`docs/module-system.md` section 5). Without this, "plug and play" is a promise; with it, `paperos module conformance migration --impl a,b` is a fact the gates can check.

**Scope**

In:

* `packages/contracts/migration/conformance/`: `defineConformanceSuite('migration', factory)` covering every port exported at v0.1, one `describe` per port, one `it` per fixture; `unsupported` capability handling so partial adapters report honestly.
* `fixtures/`: three source snapshots (CSV, Airtable metadata, Notion database) with expected mappings, two import runs (commit, rollback), one export archive that round-trips, one template pack; every fixture validated against the contract schemas in CI.
...
* Every case id is stable (`migration.<port>.<n>`) so shadow-run diffs and Linear comments can cite it.

**Interface contract**

Provides: `@paperos/contract-migration/conformance` (`defineConformanceSuite`, `memory` doubles, `fixtures` index, `capabilities` list), `conformance.json` artefact for this module. Consumes: `@paperos/contract-migration@0.1` (`module/migration/contract`), the runner and artefact schema (`module-system/conformance-runner`, PAP-239), the PAP-42 compose stack for the real-adapter run, `callAs` from PAP-268 where routes are exercised. Consumed by: `module/migration/wire`, the swap CLI, Gate 1, and every future implementation of the contract.

**Test plan**

* The suite passes against the memory doubles in CI on every PR that touches the contract or the module.
* The suite passes against the real implementation (PAP-347, PAP-348, PAP-349) on the compose stack nightly; failures open a Linear comment on the implementation issue with the case ids.
...
* Registered with the runner; Gate 1 step `conformance:migration` green; `conformance.json` attached to the PR.
* `docs/platform/contracts/migration.md` gains a "Conformance" section listing case ids; Linear comment with the report.
...
Blocked by `module/migration/contract` and `module-system/conformance-runner`. Blocks `module/migration/wire`. Soft: PAP-347, PAP-348, PAP-349 for the real-adapter run (the memory run is enough to merge).
...
Reviewer runs the suite against the CSV connector and the Airtable connector (recorded HTTP fixtures); both pass `discover`, `stream`, mapping and rollback semantics. Two minutes.

---
identifier: "PAP-468"
title: "Conformance test suite for pm-linear contract"
project: "pm-linear"
...
Give `@paperos/contract-pm-linear` an executable meaning: a Vitest suite that any implementation of the pm-linear ports must pass, plus the golden fixtures both old and new implementations are diffed against during a swap (`docs/module-system.md` section 5). Without this, "plug and play" is a promise; with it, `paperos module conformance pm-linear --impl a,b` is a fact the gates can check.

**Scope**

In:

* `packages/contracts/pm-linear/conformance/`: `defineConformanceSuite('pm-linear', factory)` covering every port exported at v0.1, one `describe` per port, one `it` per fixture; `unsupported` capability handling so partial adapters report honestly.
* `fixtures/`: twelve issues covering every state, three claim races (two sessions, one wins), four webhook payloads from Linear, one Needs Justin card, one relation graph with a cycle; every fixture validated against the contract schemas in CI.
...
* Every case id is stable (`pm-linear.<port>.<n>`) so shadow-run diffs and Linear comments can cite it.

**Interface contract**

Provides: `@paperos/contract-pm-linear/conformance` (`defineConformanceSuite`, `memory` doubles, `fixtures` index, `capabilities` list), `conformance.json` artefact for this module. Consumes: `@paperos/contract-pm-linear@0.1` (`module/pm-linear/contract`), the runner and artefact schema (`module-system/conformance-runner`, PAP-239), the PAP-42 compose stack for the real-adapter run, `callAs` from PAP-268 where routes are exercised. Consumed by: `module/pm-linear/wire`, the swap CLI, Gate 1, and every future implementation of the contract.

**Test plan**

* The suite passes against the memory doubles in CI on every PR that touches the contract or the module.
* The suite passes against the real implementation (PAP-281, PAP-372) on the compose stack nightly; failures open a Linear comment on the implementation issue with the case ids.
...
* Registered with the runner; Gate 1 step `conformance:pm-linear` green; `conformance.json` attached to the PR.
* `docs/platform/contracts/pm-linear.md` gains a "Conformance" section listing case ids; Linear comment with the report.
...
Blocked by `module/pm-linear/contract` and `module-system/conformance-runner`. Blocks `module/pm-linear/wire`. Soft: PAP-281, PAP-372 for the real-adapter run (the memory run is enough to merge).
...
Reviewer runs the suite against the Linear adapter (read-only on a sandbox team) and the in-memory PM adapter; both pass `list`, `get`, `claim` semantics; the memory adapter proves atomic claim under ten concurrent callers. Two minutes.

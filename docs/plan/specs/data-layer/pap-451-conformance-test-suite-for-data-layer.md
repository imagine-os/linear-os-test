---
identifier: "PAP-451"
title: "Conformance test suite for data-layer contract"
project: "data-layer"
...
Give `@paperos/contract-data-layer` an executable meaning: a Vitest suite that any implementation of the data-layer ports must pass, plus the golden fixtures both old and new implementations are diffed against during a swap (`docs/module-system.md` section 5). Without this, "plug and play" is a promise; with it, `paperos module conformance data-layer --impl a,b` is a fact the gates can check.

**Scope**

In:

* `packages/contracts/data-layer/conformance/`: `defineConformanceSuite('data-layer', factory)` covering every port exported at v0.1, one `describe` per port, one `it` per fixture; `unsupported` capability handling so partial adapters report honestly.
* `fixtures/`: twenty envelope samples (valid, unregistered topic, wrong version), six `FilterTree` queries with expected SQL, four job payloads, three file lifecycle traces, two tenant contexts (one missing, fails closed); every fixture validated against the contract schemas in CI.
...
* Every case id is stable (`data-layer.<port>.<n>`) so shadow-run diffs and Linear comments can cite it.

**Interface contract**

Provides: `@paperos/contract-data-layer/conformance` (`defineConformanceSuite`, `memory` doubles, `fixtures` index, `capabilities` list), `conformance.json` artefact for this module. Consumes: `@paperos/contract-data-layer@0.1` (`module/data-layer/contract`), the runner and artefact schema (`module-system/conformance-runner`, PAP-239), the PAP-42 compose stack for the real-adapter run, `callAs` from PAP-268 where routes are exercised. Consumed by: `module/data-layer/wire`, the swap CLI, Gate 1, and every future implementation of the contract.

**Test plan**

* The suite passes against the memory doubles in CI on every PR that touches the contract or the module.
* The suite passes against the real implementation (PAP-267, PAP-268, PAP-43) on the compose stack nightly; failures open a Linear comment on the implementation issue with the case ids.
...
* Registered with the runner; Gate 1 step `conformance:data-layer` green; `conformance.json` attached to the PR.
* `docs/platform/contracts/data-layer.md` gains a "Conformance" section listing case ids; Linear comment with the report.
...
Blocked by `module/data-layer/contract` and `module-system/conformance-runner`. Blocks `module/data-layer/wire`. Soft: PAP-267, PAP-268, PAP-43 for the real-adapter run (the memory run is enough to merge).
...
Reviewer runs the suite against Drizzle on the compose stack and against the in-memory doubles; both pass; the reviewer then deletes the idempotency check in the memory adapter and the suite names the failing fixture. Two minutes.

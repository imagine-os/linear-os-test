...
* Out: the task set (PAP-309), the LLM judge, trend and regression issues (PAP-310).
...
* Runner: unpack fixtures into `/srv/evals/<run>/<task>`, call `launchSession` from PAP-282 with `sandbox: true` when PAP-280 exists, `model` and `effort` overrides, capture transcript, cost (PAP-98 `result`), duration and denied-tool events (PAP-107).
...
* Requires: PAP-282 `launchSession`, PAP-106 bundles, PAP-98 cost, PAP-111 cap, PAP-107 events.
...
Blocked by PAP-104, PAP-105 (through the parent). Uses PAP-282, PAP-106, PAP-98, PAP-111.

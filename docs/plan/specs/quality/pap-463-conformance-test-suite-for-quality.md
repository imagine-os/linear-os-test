---
identifier: "PAP-463"
title: "Conformance test suite for quality contract"
project: "quality"
...
Give `@paperos/contract-quality` an executable meaning: a Vitest suite that any implementation of the quality ports must pass, plus the golden fixtures both old and new implementations are diffed against during a swap (`docs/module-system.md` section 5). Without this, "plug and play" is a promise; with it, `paperos module conformance quality --impl a,b` is a fact the gates can check.

**Scope**

In:

* `packages/contracts/quality/conformance/`: `defineConformanceSuite('quality', factory)` covering every port exported at v0.1, one `describe` per port, one `it` per fixture; `unsupported` capability handling so partial adapters report honestly.
* `fixtures/`: one artefact per gate (passing and failing), twelve findings across the severity taxonomy, two RC manifests, one seed and reset trace; every fixture validated against the contract schemas in CI.
...
* Every case id is stable (`quality.<port>.<n>`) so shadow-run diffs and Linear comments can cite it.

**Interface contract**

Provides: `@paperos/contract-quality/conformance` (`defineConformanceSuite`, `memory` doubles, `fixtures` index, `capabilities` list), `conformance.json` artefact for this module. Consumes: `@paperos/contract-quality@0.1` (`PAP-462`), the runner and artefact schema (`PAP-441`, PAP-239), the PAP-42 compose stack for the real-adapter run, `callAs` from PAP-268 where routes are exercised. Consumed by: `PAP-464`, the swap CLI, Gate 1, and every future implementation of the contract.

**Test plan**

* The suite passes against the memory doubles in CI on every PR that touches the contract or the module.
* The suite passes against the real implementation (PAP-243, PAP-248) on the compose stack nightly; failures open a Linear comment on the implementation issue with the case ids.
...
* Registered with the runner; Gate 1 step `conformance:quality` green; `conformance.json` attached to the PR.
* `docs/platform/contracts/quality.md` gains a "Conformance" section listing case ids; Linear comment with the report.
...
Blocked by `PAP-462` and `PAP-441`. Blocks `PAP-464`. Soft: PAP-243, PAP-248 for the real-adapter run (the memory run is enough to merge).

**Agent**

Built by Sentinel (Quality Lead, conformance owner) with Sentinel supplying port semantics. Reviewed by Sentinel and Atlas.
...
Reviewer runs the suite against the GitHub Actions runner adapter and the Forgejo Actions adapter on the same PR; both produce artefacts that pass the schemas and the `GateRunnerPort` fixtures. Three minutes.

---
identifier: "PAP-495"
title: "Conformance test suite for libraries contract"
project: "libraries"
...
parent: null
children: []
blockedBy: ["PAP-441", "PAP-493", "PAP-541"]
blocks: ["PAP-497"]
key: "module/libraries/conformance"
url: "https://linear.app/paperos/issue/PAP-495/conformance-test-suite-for-libraries-contract"
source: "Linear snapshot 2026-09-18T14:58Z (plan/linear-snapshot-live.json)"
updatedAt: "2026-09-18T14:21:00.183Z"
model: "claude-opus-5"
effort: "high"
estimate: 3
dueDate: "2026-09-24"
...
# PAP-495: Conformance test suite for libraries contract

**Model / Effort:** Opus 5 / high

**Goal**

Give `@paperos/contract-libraries` an executable meaning: a Vitest suite that any implementation of the libraries ports must pass, plus the golden fixtures both old and new implementations are diffed against during a swap (`docs/module-system.md` section 5). Without this, "plug and play" is a promise; with it, `paperos module conformance libraries --impl a,b` is a fact the gates can check.

**Scope**

In:

* `packages/contracts/libraries/conformance/`: `defineConformanceSuite('libraries', factory)` covering every port exported at v0.1, one `describe` per port, one `it` per fixture; `unsupported` capability handling so partial adapters report honestly.
* `fixtures/`: twelve library records (four per status), three ADRs, the license policy with one allowed, one review and one blocked package, two Scout reports; every fixture validated against the contract schemas in CI.
...
* Every case id is stable (`libraries.<port>.<n>`) so shadow-run diffs and Linear comments can cite it.

**Interface contract**

Provides: `@paperos/contract-libraries/conformance` (`defineConformanceSuite`, `memory` doubles, `fixtures` index, `capabilities` list), `conformance.json` artefact for this module. Consumes: `@paperos/contract-libraries@0.1` (`module/libraries/contract`), the runner and artefact schema (`module-system/conformance-runner`, PAP-239), the PAP-42 compose stack for the real-adapter run, `callAs` from PAP-268 where routes are exercised. Consumed by: `module/libraries/wire`, the swap CLI, Gate 1, and every future implementation of the contract.

**Test plan**

* The suite passes against the memory doubles in CI on every PR that touches the contract or the module.
* The suite passes against the real implementation (PAP-216, PAP-211) on the compose stack nightly; failures open a Linear comment on the implementation issue with the case ids.
...
* Registered with the runner; Gate 1 step `conformance:libraries` green; `conformance.json` attached to the PR.
* `docs/platform/contracts/libraries.md` gains a "Conformance" section listing case ids; Linear comment with the report.
...
Blocked by `module/libraries/contract` and `module-system/conformance-runner`. Blocks `module/libraries/wire`. Soft: PAP-216, PAP-211 for the real-adapter run (the memory run is enough to merge).

**Agent**

Built by Sentinel (Quality Lead, conformance owner) with Scout supplying port semantics. Reviewed by Scout and Atlas.
...
Reviewer runs the suite against the docs-engine registry adapter and a JSON-file adapter; both pass record validation, status transitions and license policy fixtures. Ninety seconds.

---
identifier: "PAP-480"
title: "Wire collab behind the module registry with an adapter and feature flag"
project: "collab"
...
Make `collab` a swappable module in practice: its implementation binds into the `@paperos/kernel` registry as the default adapter for `@paperos/contract-collab`, every consumer resolves the ports from the kernel instead of importing `packages/collab (docs, comments, notifications, prompt log), docs/`, and a `module.collab.impl` variant flag can select a second implementation per tenant with shadow-run and five-second rollback (`docs/module-system.md` sections 4 and 6). After this issue, rewriting collab touches no other module.

**Scope**

In:

* `packages/<impl>/src/adapter.ts` (or one file per port): classes or factories implementing each port from `@paperos/contract-collab`, registered by `module.ts` `bind(kernel)`; request-scoped ports receive the tenant and actor from the kernel's request scope.
* Consumer migration: every import of `collab` internals from another module (found by `pnpm gen:dep-map --undeclared`) replaced with `kernel.resolve(port)` or a hook from the contract; lint R8 turned from warning to error for this module.
* Flag `module.collab.impl` declared in `flags.yaml` (PAP-366) with variants `default` and `next`; a `next` implementation stub that re-exports the default (so the mechanism is exercised before a real rewrite exists).
...
Provides: `collab` bound in the kernel as `@paperos/contract-collab` provider `default`, flag `module.collab.impl`, manifest `slots.fills` and `events.subscribes` complete, `X-PaperOS-Impl` on this module's routes. Consumes: `@paperos/kernel` registry and DI (`module-system/registry-di`), flag swap mechanism (`module-system/flag-swap`), `@paperos/contract-collab@0.1` and its conformance suite, the implementation from PAP-317, PAP-128, PAP-129, PAP-366 flags, PAP-267 middleware for the header. Consumed by: every consumer of the contract (PAP-135 (prompt log browser), PAP-137 (screenshot annotations), PAP-138 (unified search), PAP-410 (support inbox), PAP-100 PM comments, PAP-216 library registry), the swap CLI, PAP-266's removal matrix.

**Test plan**

* Unit: `bind` registers every port; unbound port fails boot with its name; request scope carries tenant and actor.
* Integration (compose stack): conformance suite passes against the kernel-resolved provider (not a direct import); flipping `module.collab.impl` to `next` for one tenant changes `X-PaperOS-Impl` for that tenant only within 5 s; kill switch returns `default`.
* Static: `pnpm gen:dep-map --undeclared` reports zero undeclared edges into `collab`; lint R8 error level; `knip` finds no unused exports left behind by the migration.
...
* Kernel binding merged; zero undeclared imports into `collab` across the monorepo; conformance green through the kernel; flag flip recording attached; shadow diff report empty.
...
Built by Nova (In-App Collaboration & Knowledge owner). Reviewed by Quill and Forge (kernel).
...
Reviewer flips `module.collab.impl` to `memory` in a test app and opens the record panel: the comments tab still renders (empty) and posting a comment works for the session, proving the tables module never imported collab internals. Ninety seconds.
